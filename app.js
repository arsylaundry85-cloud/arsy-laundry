...
document.addEventListener("DOMContentLoaded", function () {
  ...
  fixStyle.innerHTML = `
    * {
    ...
    body, button, .nav-button, .transaction-item, .service-item, .card, div {
      -webkit-user-select: none;
      user-select: none;
      -webkit-touch-callout: none;
    }
    .nav-button, button, [onclick] {
      cursor: pointer;
  ...
  setupAkunOutletLink();
  setupDirectNavClicks();
  removeProElements();
});

function setupDirectNavClicks() {
  const navButtons = document.querySelectorAll(".nav-button");
  if (navButtons.length >= 4) {
    navButtons[0].onclick = () => showPage('dashboardPage');
    navButtons[1].onclick = () => showPage('transactionsPage');
    navButtons[2].onclick = () => showPage('reportPage');
    navButtons[3].onclick = () => showPage('akunPage');
  } else {
    document.querySelectorAll(".nav-button, button").forEach(btn => {
      const text = btn.textContent.trim().toLowerCase();
      if (text.includes("dashboard")) btn.onclick = () => showPage('dashboardPage');
      else if (text.includes("transaksi")) btn.onclick = () => showPage('transactionsPage');
      else if (text.includes("laporan")) btn.onclick = () => showPage('reportPage');
      else if (text.includes("akun")) btn.onclick = () => showPage('akunPage');
    });
  }
}
...
document.addEventListener("DOMContentLoaded", function () {
  // Inject CSS to prevent text selection popup on mobile taps
...
function setupBottomNavFix() {
  document.querySelectorAll(".nav-button").forEach(btn => {
    let page = btn.getAttribute("onclick") || btn.dataset.page;
    if (!page) {
      const text = btn.textContent.trim().toLowerCase();
      if (text.includes("dashboard")) btn.onclick = () => showPage('dashboardPage');
      else if (text.includes("transaksi")) btn.onclick = () => showPage('transactionsPage');
  ...
  modal.innerHTML = `
    <div class="modal-content" style="background: white; padding: 20px; border-radius: 16px; width: 90%; max-width: 360px; box-shadow: 0 4px 20px rgba(0,0,0,0.15);">
      ...
      <form id="editTransactionItemForm" onsubmit="saveEditTransactionItem(event)">
        ...
        <div style="background: #f8fafc; padding: 12px; border-radius: 8px; margin-bottom: 16px; display: flex      }
      renderAll();
    }
  } catch (err) {
    console.log("Gagal memuat dari cloud, menggunakan data lokal.");
...
function setupDashboardInteractions() {
  ...
  targets.forEach(({ id, handler }) => {
    ...
    // Improved robust selector to make the entire card box reliably clickable
    let card = el.parentElement;
    for (let i = 0; i < 3; i++) {
      if (card && card.parentElement && card.parentElement.id !== 'dashboardPage') {
...
function saveRichService(e) {
  ...
  saveServicesData();
  saveData(); // <--- KODE TAMBAHAN UNTUK FIX
  renderServices();
  closeServiceModal();
  showToast("Layanan berhasil diperbarui");
}
...
function generateWhatsAppReceiptText(item) {
  ...
  itemsList.forEach(it => {
    ...
    text += `${it.serviceType}\n${berat} x ${formatRupiah(hargaPerUnit)} = ${formatRupiah(item.total)}\n`;
  });
